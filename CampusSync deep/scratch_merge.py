...
# 1. Update the UI Column inside HomeScreen
old_column_start = content.find('Column(\n        modifier = Modifier\n            .fillMaxSize()')
if old_column_start == -1:
    old_column_start = content.find('Column(\n        modifier = Modifier.fillMaxSize()')

# Find the end of HomeScreen (before AnnouncementsModal)
old_column_end = content.find('    if (showAnnouncementsDialog) {', old_column_start)

if old_column_start == -1 or old_column_end == -1:
    print("Could not find column boundaries")
    exit(1)

premium_column = '''Column(
...
content = content[:old_column_start] + premium_column + content[old_column_end:]

# 2. Add the premium components at the end of the file
premium_components = '''
